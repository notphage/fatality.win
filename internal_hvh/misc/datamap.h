...
{ \
	...
	if(!offset) { _( s, datamapname ); offset = find_in_data_map(this->get_pred_desc_map(), s ); \
	Assert(offset); } \
...
{ \
	static uint16_t offset = 0; \
	if(!offset) { _( s, datamapname ); offset = find_in_data_map(this->get_pred_desc_map(), datamapname); \
	Assert(offset); } \
